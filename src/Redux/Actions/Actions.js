...
export const getGruposByTropa = (tropa) => {
  return async (dispatch) => {
    try {
      ...
      json.data.data.detalle.forEach(a => {
        if (a.categoria === "Vaca") {
          kgVaca += a.kg
          nvaca++
        }
...
export const getGruposCerdoByTropa = (tropa) => {
  return async (dispatch) => {
    try {
      const json = await axios.get(`/faenas/${tropa}`, {
        headers: {
      ...
      let kgCapon = 0;
      let nCapon = 0;
      json.data.data.detalle.forEach(res => {
        if (res.categoria === "Chancha") {
          kgChancha += +res.kg
          nChancha++
        }
        if (res.categoria === "Capon") {
          kgCapon += +res.kg
          nCapon++
        }
      })
